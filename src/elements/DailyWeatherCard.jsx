    ...
    return (
        <StyledCard className="forecast__card flex-col">
            ...
            <div className="daily__minmax flex-row">
                <p className="minmax max">{Math.floor(props.details.max_temp)}{props.tempFormat === 0 ? "ºC" : "ºF"}</p>
                <p className="minmax min">{Math.floor(props.details.min_temp)}{props.tempFormat === 0 ? "ºC" : "ºF"}</p>
            </div>
        </StyledCard>
    )
}